...
class LabelListWidget(QtWidgets.QListView):
    ...
    def updateLabelList(self, shapes):
        ...
        for i, shape in enumerate(shapes):
            ...
            #     label_list_item.setBackground(shape.fill_color)

            # 添加到列表
            self.addItem(label_list_item)

    def updateLabelColor(self, _label_id_list):  # 2025-2-13 高亮
        for _idx in _label_id_list:
            item_shape = self.model().item(_idx, 0)
            item_shape.setBackground(QColor(Qt.yellow))
