...
from PyQt5.QtCore import pyqtSignal  # 2025-2-13
...
class Canvas(QtWidgets.QWidget):
    ...
    updateLabelListRequested = pyqtSignal(list)  # 定义信号  2025-2-13
    updateLabelColorRequested = pyqtSignal(list)  # 定义信号
    canvasErrorMessageRequested = pyqtSignal(str, str)  # 定义信号
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.selected_shape_index = None  # 记录第一个双击的 shape 的索引
        self.double_click_enabled = True  # 启用双击功能
        if self.double_click not in [None, "close"]:
            raise ValueError(
                "Unexpected value for double_click event: {}".format(self.double_click)
            )
        self.num_backups = kwargs.pop("num_backups", 20)
    ...
    def moveShapeAfter(self, from_index, to_index): # 2025-2-13
        """将 from_index 的 shape 移动到 to_index 的后面"""
        # if from_index == to_index:
        #     return
        # if self.shapes[int(to_index)].label != 1:
        #     # print('双击第一个shape类型不为1，请重新选择')
        #     self.canvasErrorMessageRequested.emit('请注意：', '双击的第一个shape类型不为1，请重新选择')
        #     return
        # if self.shapes[int(from_index)].label <= 1:
        #     self.canvasErrorMessageRequested.emit('请注意：', '双击第二个shape的类型应该大于1，请重新选择第一个类型')
        #     # print('双击第二个shape的类型应该大于1')
        #     return
        shape = self.shapes.pop(from_index)
        if from_index < to_index:
            self.shapes.insert(to_index, shape)
        else:
            self.shapes.insert(to_index + 1, shape)
        print('移动成功')

    def isPointInsidePolygon(self, point, polygon): # 2025-2-13
        """判断点是否在多边形内"""
        x, y = point.x(), point.y()
        n = len(polygon)
        inside = False
    ...
    def isPointInsideRectangle(self, point, shape):  # 2025-2-13
        """判断点是否在矩形内"""
    ...
    def mouseDoubleClickEvent(self, ev):  # 2025-2-13
        # 新逻辑：双击移动 shape
        if self.mode == self.EDIT:  # 仅在编辑模式下启用新逻辑
            # print('编辑模式')
            pos = self.transformPos(ev.pos())
            shape_index = None
            for i, shape in enumerate(self.shapes):
                if shape.shape_type == "rectangle" and self.isPointInsideRectangle(pos, shape):
                    shape_index = i
                    break
            if shape_index is not None:
                if self.selected_shape_index is None:
                    # print(shape_index, self.shapes[int(shape_index)].label, type(self.shapes[int(shape_index)].label))
                    # 第一次双击，记录 shape 的索引
                    if self.shapes[int(shape_index)].label != '1':
                        self.canvasErrorMessageRequested.emit('请注意：', '双击的第一个shape类型不为1，请重新选择')
                        return
                    self.selected_shape_index = shape_index
                        # print(f"First shape selected: {shape_index}")
                    self.updateLabelColorRequested.emit([shape_index])
    ...
    def paintEvent(self, event):
        ...
        for shape in self.shapes:
            if shape.shape_type == "rectangle":
                # 获取框的坐标（QPointF 对象）
                point1 = shape.points[0]  # 第一个点
                point2 = shape.points[1]  # 第二个点
                x1 = point1.x()  # 提取 x 坐标
                y1 = point1.y()  # 提取 y 坐标
                x2 = point2.x()  # 提取 x 坐标
                y2 = point2.y()  # 提取 y 坐标
        ...
        if self.selectedShapesCopy:
            for idx, s in enumerate(self.selectedShapesCopy):
                s.paint(p)
    ...
    def finalise(self):
        assert self.current
        # 获得最大shape_id
        max_id = 0 if len(self.shapes) == 0 else max([sha.shape_id for sha in self.shapes]) # 2025-2-13
        self.current.shape_id = max_id + 1 # 2025-2-13
