...
LABEL_COLORMAP[:12] = [
    (0, 0, 0),  # 0: unused, label ids start at 1 (see _get_rgb_by_label)
    (230, 25, 75),  # 1: red
    (67, 99, 216),  # 2: blue
    (60, 180, 75),  # 3: green
    (255, 130, 48),  # 4: orange
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        # 连接信号和槽
        self.canvas.updateLabelListRequested.connect(self.labelList.updateLabelList)  # 2025-2-13
        self.canvas.updateLabelColorRequested.connect(self.labelList.updateLabelColor)  # 2025-2-13
        self.canvas.canvasErrorMessageRequested.connect(self.errorMessage) # 2025-2-17
        ...
        utils.addActions(self.menus.tool, (replace23,))
        utils.addActions(
            self.menus.view,
            (
    ...
    def replace23(self):
        label_filepath = osp.splitext(self.imagePath)[0] + ".json"
        print('替换文件中标签2为3label_filepath:', label_filepath)
        with open(label_filepath, "r") as f:
            data = json.loads(f.read())
        start_i = 0
        while start_i < len(data["shapes"]):
            if data["shapes"][start_i]["label"] == "2":
                data["shapes"][start_i]["label"] = "3"
            start_i = start_i + 1
        with open(label_filepath, "w") as f:
            f.write(json.dumps(data))
        self.errorMessage("请注意：", f'已替换文件中标签2为3{label_filepath}，请按上一页或下一页更新。')
    ...
    def loadLabels(self, shapes):
        ...
        for idx, shape in enumerate(shapes):
            ...
            group_id = shape["group_id"]
            shape_id = shape["shape_id"] if "shape_id" in shape and shape["shape_id"] >= 0 else idx + 1
    ...
    def duplicateSelectedShape(self):
        ...
        self.labelList.clearSelection()
        # 获得最大shape_id
        max_id = 0 if len(self.canvas.shapes) == 0 else max([sha.shape_id for sha in self.canvas.shapes])  # 2025-2-13
        for idx, shape in enumerate(added_shapes):  # 2025-2-13
            shape.shape_id = max_id + idx + 1
            self.addLabel(shape)
        self.setDirty()
